...
So if your profile is `github.com/priyasharma`, type `priyasharma.md`.
...
Tap the big empty area and type this, changing every line to be about you:
...
thing most people get wrong: `priya-sharma.md` with `github: priyasharma`
inside it will fail the check.

`name`, `github` and `branch` are required. `year` and `building` are
optional. The sentences underneath can be up to 500 characters.
